...
export const PROJECTS = [
  {
    ...
    image: project1,
    description:
      "Developed an advanced Telegram-based automation bot designed to simplify and accelerate file mirroring, downloading, and management across multiple cloud platforms. The bot integrates the Google Drive API and Rclone API to support high-speed mirroring to Google Drive, Mega, and other remote destinations, reducing manual upload efforts by up to 80%. It supports downloading from torrents, direct links, and streaming sites using Aria2c, Qbittorrent, and YT-DLP, and includes bypass mechanisms for protected links and support for multiple URL shortener services.\n\nA custom FFmpeg-powered media toolkit enables pre-processing operations such as trimming, merging, format conversion, thumbnail generation, and compression before upload — reducing manual editing overhead by 30–40% and ensuring consistent media quality across uploads. To enhance functionality further, implemented manual and automated poster/video thumbnail scraping from TMDB, Netflix, and Amazon Prime using FuzzyWuzzy-based title matching, improving thumbnail accuracy and presentation quality while reducing manual effort by approximately 60–70%.\n\nThe bot also includes API-based scraping and link extraction for popular sites (Cinevood, Movies4u, SkyMoviesHD, etc.), automatically extracting direct download links and generating hash/direct link references — cutting content preparation time by over 50%. An RSS-driven automation pipeline continuously monitors configured feeds and triggers complete download-leech workflows on new releases for unattended 24/7 operation. User data, settings, and task states are persisted in MongoDB, enabling reliable resume/retry and per-user preferences.\n\nBeyond core mirroring, the project includes a web-connected streaming UI (JavaScript + Telegram indexing) allowing previews and streaming directly from the bot, and a comprehensive miscellaneous toolkit within Telegram: OCR (Optical Character Recognition), TTS (Text-to-Speech), WebSS (Website Screenshot), Translate (message translation to supported languages), Pahe search integration, VidSS (video screenshot/thumbnails), Thumb generation, and general media conversion utilities. The system is containerized with Docker for reproducible deployment and supports hosting on Heroku, VPS, or Docker-enabled environments. Overall, the Mirror Leech Telegram Bot is a production-grade, scalable leeching and media automation ecosystem optimized for speed, reliability, and user experience.",
    technologies: [
      "Python",
      "JavaScript",
      "MongoDB",
